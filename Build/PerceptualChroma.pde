...
static class PerceptualChroma {
    ...
    public static float[] getRGB(float huslTuple[]) {
        ...
        // println("Convert HUSL ---> RGB");

        result[0] *= 255;
        result[1] *= 255;
        result[2] *= 255;
        // println("RGB: " + (int)result[0] + " / " + (int)result[1]+ " / " +  (int)result[2]);
        return result;
    }
    ...
    */
    ...
    public static float maxChroma(float L, float H) {
        ...
        for (int channel = 0; 3 != channel; channel++) {
            final float[] channelM = m[channel];
